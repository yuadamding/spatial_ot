...
The historical OT-dictionary assignment is still available as `--subregion-clustering-method ot_dictionary`, and the composition-summary baseline is available as `--subregion-clustering-method pooled_subregion_latent`. The packaged cohort workflow now defaults to `heterogeneity_descriptor_niche`. OT atoms and spot-latent charts remain downstream diagnostics/projections conditioned on the fitted subregion labels.
