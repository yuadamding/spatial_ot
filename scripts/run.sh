...
MAX_SPOT_LATENT_PLOT_OCCURRENCES="${MAX_SPOT_LATENT_PLOT_OCCURRENCES:-0}"
WRITE_CONCERN_REPORT="${WRITE_CONCERN_REPORT:-1}"
STRICT_CONCERN_REPORT="${STRICT_CONCERN_REPORT:-0}"
CONCERN_COORDINATE_BASELINE_RUN_DIR="${CONCERN_COORDINATE_BASELINE_RUN_DIR:-}"
CONCERN_STABILITY_RUN_DIRS="${CONCERN_STABILITY_RUN_DIRS:-}"
...
CUDA_TARGET_VRAM_GB="${CUDA_TARGET_VRAM_GB:-70}"
...
DEEP_INDEPENDENCE_WEIGHT="${DEEP_INDEPENDENCE_WEIGHT:-0.1}"
...
DEEP_CHECKPOINT_DIR="${DEEP_CHECKPOINT_DIR:-}"
DEEP_CHECKPOINT_EVERY_EPOCHS="${DEEP_CHECKPOINT_EVERY_EPOCHS:-5}"
DEEP_RESUME_CHECKPOINT="${DEEP_RESUME_CHECKPOINT:-}"
DEEP_RESUME_RESET_OPTIMIZER_LR="${DEEP_RESUME_RESET_OPTIMIZER_LR:-0}"
...
export SPATIAL_OT_SUBREGION_LATENT_EMBEDDING_MODE="${SPATIAL_OT_SUBREGION_LATENT_EMBEDDING_MODE:-$SUBREGION_LATENT_EMBEDDING_MODE}"
export SPATIAL_OT_SUBREGION_LATENT_SHRINKAGE_TAU="${SPATIAL_OT_SUBREGION_LATENT_SHRINKAGE_TAU:-$SUBREGION_LATENT_SHRINKAGE_TAU}"
...
DEEP_CHECKPOINT_DIR="${DEEP_CHECKPOINT_DIR:-${OUTPUT_DIR}/deep_checkpoints}"
...
if [[ -n "$MAX_SUBREGION_AREA_UM2" ]]; then
  EXTRA_FLAGS+=(--max-subregion-area-um2 "$MAX_SUBREGION_AREA_UM2")
fi
EXTRA_FLAGS+=(
...
if [[ "$DEEP_FEATURE_METHOD" != "none" ]]; then
  DEEP_FLAGS+=(
    ...
    --deep-independence-weight "$DEEP_INDEPENDENCE_WEIGHT"
    --deep-checkpoint-dir "$DEEP_CHECKPOINT_DIR"
    --deep-checkpoint-every-epochs "$DEEP_CHECKPOINT_EVERY_EPOCHS"
  )
  if [[ -n "$DEEP_GRADIENT_CLIP_NORM" ]]; then
    DEEP_FLAGS+=(--deep-gradient-clip-norm "$DEEP_GRADIENT_CLIP_NORM")
  fi
  ...
  if [[ -n "$DEEP_RESUME_CHECKPOINT" ]]; then
    DEEP_FLAGS+=(--deep-resume-checkpoint "$DEEP_RESUME_CHECKPOINT")
  fi
  if [[ "$DEEP_RESUME_RESET_OPTIMIZER_LR" == "1" ]]; then
    DEEP_FLAGS+=(--deep-resume-reset-optimizer-lr)
  else
    DEEP_FLAGS+=(--no-deep-resume-reset-optimizer-lr)
  fi
fi
...
if [[ "$WRITE_CONCERN_REPORT" == "1" ]]; then
  CONCERN_FLAGS=(--run-dir "$OUTPUT_DIR")
  if [[ -n "$CONCERN_COORDINATE_BASELINE_RUN_DIR" ]]; then
    CONCERN_FLAGS+=(--coordinate-baseline-run-dir "$CONCERN_COORDINATE_BASELINE_RUN_DIR")
  fi
  if [[ -n "$CONCERN_STABILITY_RUN_DIRS" ]]; then
    IFS=',' read -r -a CONCERN_STABILITY_DIR_ARRAY <<< "$CONCERN_STABILITY_RUN_DIRS"
    for concern_dir in "${CONCERN_STABILITY_DIR_ARRAY[@]}"; do
      if [[ -n "$concern_dir" ]]; then
        CONCERN_FLAGS+=(--stability-run-dir "$concern_dir")
      fi
    done
  fi
  if [[ -n "$CONCERN_LEAKAGE_ABLATION_RUN_DIRS" ]]; then
    IFS=',' read -r -a CONCERN_LEAKAGE_DIR_ARRAY <<< "$CONCERN_LEAKAGE_ABLATION_RUN_DIRS"
    for concern_dir in "${CONCERN_LEAKAGE_DIR_ARRAY[@]}"; do
      if [[ -n "$concern_dir" ]]; then
        CONCERN_FLAGS+=(--leakage-ablation-run-dir "$concern_dir")
      fi
    done
  fi
  if [[ "$STRICT_CONCERN_REPORT" == "1" ]]; then
    CONCERN_FLAGS+=(--strict)
  fi
  "$PYTHON_BIN" -m spatial_ot validate-run-concerns "${CONCERN_FLAGS[@]}"
fi
