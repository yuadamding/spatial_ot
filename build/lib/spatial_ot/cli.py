...
def build_parser() -> argparse.ArgumentParser:
    ...
    spatial_validation.add_argument("--run-dir", required=True, help="Finished multilevel OT run directory.")
    spatial_validation.add_argument("--output-json", default=None, help="Optional JSON report path.")
    spatial_validation.add_argument("--output-cluster-csv", default=None, help="Optional cluster-statistics CSV path.")
    ...
    spatial_validation.add_argument("--knn", type=int, default=6, help="Neighbors per subregion for spatial homophily.")
    spatial_validation.add_argument(
        "--permutations",
        type=int,
        default=20,
    ...
    spatial_validation.add_argument("--seed", type=int, default=0, help="Random seed for subsampling/permutation.")
...
def main() -> None:
    ...
    if args.command == "plot-sample-niches":
        ...
        report = spatial_niche_validation_report(
            run_dir=Path(args.run_dir),
            output_json=Path(args.output_json) if args.output_json else None,
            output_cluster_csv=Path(args.output_cluster_csv) if args.output_cluster_csv else None,
