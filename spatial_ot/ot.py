...
def fit_state_atoms(z: np.ndarray, n_atoms: int, seed: int) -> tuple[np.ndarray, np.ndarray]:
    ...
    state_probs = _row_softmax(distances, temperature=0.5)
...
def _build_shell_profiles(data: PreparedSpatialOTData, state_probs: np.ndarray) -> np.ndarray:
    ...
    for edge_index in data.shell_edge_indices:
        ...
        np.add.at(out, dst, state_probs[src])
...
def build_neighborhood_objects(data: PreparedSpatialOTData, z: np.ndarray, s: np.ndarray, config: ExperimentConfig) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    centroids, state_probs = fit_state_atoms(z, n_atoms=n_atoms, seed=config.data.seed)
...
def fit_niche_prototypes(data: PreparedSpatialOTData, z: np.ndarray, s: np.ndarray, config: ExperimentConfig) -> NicheResult:
    ...
    for i in range(data.n_cells):
        for k in range(config.model.niche_prototypes):
            hist_dist = ot.sinkhorn2(
                _normalize_mass(hist[i]),
                _normalize_mass(prototype_hist[k]),
                atom_cost,
                reg=config.loss.ot_temperature,
                numItermax=400,
                warn=False,
            )
            cov_dist = np.mean((cov_features[i] - prototype_cov[k]) ** 2)
            if shell_features.shape[1] > 0:
                shell_ground = np.eye(shell_features.shape[1], dtype=np.float32)
                shell_dist = ot.sinkhorn2(
                    _normalize_mass(shell_features[i]),
                    _normalize_mass(prototype_shell[k]),
                    shell_ground,
                    reg=config.loss.ot_temperature,
                    numItermax=400,
                    warn=False,
    ...
    niche_probs = _row_softmax(distance_matrix, temperature=config.loss.ot_temperature)
