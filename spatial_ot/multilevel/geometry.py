...
def _geometry_progress(message: str) -> None:
    ...
    if raw in {"1", "true", "yes", "on"}:
        ...
        print(
            f"[spatial_ot geometry {elapsed:8.1f}s] {message}",
            file=sys.stderr,
            flush=True,
        )
...
def _center_from_members(coords_um: np.ndarray, members: np.ndarray) -> np.ndarray:
    return (
        np.asarray(coords_um, dtype=np.float32)[np.asarray(members, dtype=np.int32)]
        .mean(axis=0)
        .astype(np.float32)
    )
...
def _target_data_driven_region_count(
    ...
    *,
    ...
    # max_area_um2 is a QC/reporting target for generated subregions. It should
    # not force extra seeds or override the data-driven connected partition.
    del max_area_um2
    ...
    spatial_estimate = int(
        np.ceil(float(span[0] * span[1]) / max(target_scale * target_scale, 1e-6))
    )
...
def _prepare_partition_features(
    ...
    *,
    ...
    if (
        partition_features is None
        or float(feature_weight) <= 0.0
        or int(feature_dims) <= 0
    ):
...
def _fit_coordinate_seed_partition(
    ...
    *,
    ...
    batch_size = min(
        n_cells,
        max(8192, int(target_count) * _subregion_seed_kmeans_batch_multiplier()),
    )
...
def _refine_partition_by_feature_boundaries(
    ...
    *,
    ...
    refinement_iters = (
        _subregion_boundary_refinement_iters()
        if n_iters is None
        else max(0, int(n_iters))
    )
    refinement_knn = (
        _subregion_boundary_refinement_knn()
        if n_neighbors is None
        else max(2, int(n_neighbors))
    )
    if (
        partition_features is None
        or float(feature_weight) <= 0.0
        ...
        centers = np.vstack(
            [_center_from_members(coords_um, member) for member in members]
        ).astype(np.float32)
    ...
    chunk_size = max(
        50000, _env_int("SPATIAL_OT_SUBREGION_BOUNDARY_CHUNK_SIZE", 100000)
    )
    ...
    for _ in range(refinement_iters):
        ...
        centers = np.vstack(
            [_center_from_members(coords, member) for member in members_iter]
        ).astype(np.float32)
        feature_centers = np.vstack(
            _feature_centroids_from_members(features, members_iter)
        ).astype(np.float32)
        changed = 0
        ...
        for start in range(0, coords.shape[0], chunk_size):
            ...
            scores = (
                spatial_weight * spatial_score + float(feature_weight) * feature_score
            )
            assigned = candidates[
                np.arange(stop - start), np.argmin(scores, axis=1)
            ].astype(np.int32)
    ...
    out_centers = np.vstack(
        [_center_from_members(coords, member) for member in out_members]
    ).astype(np.float32)
    ...
    *,
    ...
    for member in members:
        ...
        for cid in range(int(n_components)):
            component = np.sort(
                member_arr[np.flatnonzero(component_labels == cid)]
            ).astype(np.int32)
            if component.size == 0:
                continue
...
def _split_members_by_max_area(
    *,
    ...
    coords = np.asarray(coords_um, dtype=np.float32)
    features = (
        np.asarray(partition_features, dtype=np.float32)
            ...
            if bool(allow_below_min_cells):
                ...
                if features is not None and float(feature_weight) > 0.0:
                    ...
                    local_view = np.concatenate(
                        [
                            ...
                            * _standardize_features(local_features),
                        ],
                        axis=1,
                    )
                split_members = []
                for trial_parts in range(int(n_parts), 1, -1):
                    model = MiniBatchKMeans(
                        ...
                        batch_size=min(
                            max(1024, int(trial_parts) * 8), int(member.size)
                        ),
                        n_init=_subregion_seed_kmeans_n_init(),
                        max_iter=max(_subregion_seed_kmeans_max_iter(), 20),
                        reassignment_ratio=0.0,
                    )
                    labels = model.fit_predict(local_view.astype(np.float32)).astype(
                        np.int32
                    )
                    trial_members = [
                        np.sort(member[np.flatnonzero(labels == label)]).astype(
                            np.int32
                        )
                        for label in range(int(labels.max()) + 1)
                        if np.any(labels == label)
                    ]
                    if len(trial_members) > 1 and all(
                        child.size >= split_min_cells for child in trial_members
                    ):
                        split_members = trial_members
                        break
            if len(split_members) <= 1 or any(
                _member_area_um2(coords, child) >= area * 0.999
                for child in split_members
            ):
                balanced = _balanced_long_axis_split(member, int(n_parts))
                if len(balanced) > 1 and all(
                    child.size >= split_min_cells for child in balanced
                ):
...
def _validate_max_subregion_area(
    ...
    *,
    ...
    violating = [
        (
            idx,
            _member_area_um2(coords_um, member),
            int(np.asarray(member, dtype=np.int32).size),
        )
...
def _region_adjacency_from_knn(
    ...
    *,
    ...
    nn = NearestNeighbors(
        n_neighbors=min(int(n_neighbors) + 1, n_cells), metric="euclidean"
    )
    nn.fit(coords)
    ...
    for src, dst in zip(
        row_labels[mask].tolist(), col_labels[mask].tolist(), strict=False
    ):
...
def _weighted_cluster_feature_prototypes(
    ...
    means = np.asarray(region_means, dtype=np.float32)
    sizes = np.asarray(region_sizes, dtype=np.float64)
    labels = np.asarray(region_cluster_labels, dtype=np.int32)
    n_clusters = int(labels.max(initial=-1)) + 1
    if n_clusters <= 0:
        return np.zeros((0, means.shape[1] if means.ndim == 2 else 0), dtype=np.float32)
    prototypes = np.zeros((n_clusters, means.shape[1]), dtype=np.float64)
    weights = np.zeros(n_clusters, dtype=np.float64)
    for rid, label in enumerate(labels.tolist()):
        if int(label) < 0:
            continue
        weight = max(float(sizes[int(rid)]), 1.0)
        prototypes[int(label)] += weight * means[int(rid)]
        weights[int(label)] += weight
    global_mean = (
        means.mean(axis=0).astype(np.float64)
        if means.size
        else np.zeros(means.shape[1], dtype=np.float64)
    )
    for label in range(n_clusters):
        if weights[label] > 0:
            prototypes[label] /= weights[label]
        else:
            prototypes[label] = global_mean
    return prototypes.astype(np.float32)


def _cluster_feature_objective(
    region_means: np.ndarray,
    region_sizes: np.ndarray,
    region_cluster_labels: np.ndarray,
    ...
    total = 0.0
    for rid, label in enumerate(labels.tolist()):
        if int(label) < 0 or int(label) >= proto.shape[0]:
            continue
        delta = means[int(rid)] - proto[int(label)]
        total += float(max(float(sizes[int(rid)]), 1.0) * np.mean(delta * delta))
    return float(total)
...
def refine_subregions_by_cluster_coherence(
    ...
    *,
    ...
    This is a constrained post-clustering boundary refinement step. It only
    considers moving boundary cells into adjacent kNN regions, keeps source
    regions above ``min_cells`` during the move sweep, then enforces final
    connected components and minimum size by splitting/merging. It is meant to
    couple segmentation to the downstream subregion-cluster model without
    allowing unconstrained feature-only segmentation.
    """
    coords = np.asarray(coords_um, dtype=np.float32)
    feat = np.asarray(features, dtype=np.float32)
    if coords.ndim != 2 or coords.shape[1] != 2:
        raise ValueError("coords_um must have shape (n_cells, 2).")
    if feat.ndim != 2 or feat.shape[0] != coords.shape[0]:
        raise ValueError("features must have shape (n_cells, n_features).")
    n_cells = int(coords.shape[0])
    if n_cells == 0 or len(members) == 0:
        raise RuntimeError("Cannot refine an empty subregion partition.")
    labels = _labels_from_partition(n_cells, members)
    _validate_mutually_exclusive_memberships(
        n_cells, members, require_full_coverage=True
    )
    region_cluster_labels = np.asarray(subregion_cluster_labels, dtype=np.int32)
    if region_cluster_labels.shape[0] != len(members):
        raise ValueError(
            "subregion_cluster_labels must contain one label per subregion."
        )

    keep_dims = min(max(int(feature_dims), 1), int(feat.shape[1]))
    feature_view = feat[:, :keep_dims].astype(np.float32, copy=False)
    if int(n_iters) <= 0 or float(max_move_fraction) <= 0.0 or len(members) < 2:
        centers = np.vstack(
            [_center_from_members(coords, member) for member in members]
        ).astype(np.float32)
        return (
            centers,
            [np.asarray(member, dtype=np.int32) for member in members],
            [np.asarray([], dtype=np.int32) for _ in members],
            [],
        )

    nn = NearestNeighbors(
        n_neighbors=min(max(int(n_neighbors), 2) + 1, n_cells), metric="euclidean"
    )
    nn.fit(coords)
    ...
    for iteration in range(max(int(n_iters), 0)):
        n_regions = int(labels.max(initial=-1)) + 1
        region_sizes = np.bincount(labels, minlength=n_regions).astype(np.float64)
        feature_sums = np.zeros((n_regions, keep_dims), dtype=np.float64)
        coord_sums = np.zeros((n_regions, 2), dtype=np.float64)
        np.add.at(feature_sums, labels, feature_view.astype(np.float64))
        np.add.at(coord_sums, labels, coords.astype(np.float64))
        denom = np.maximum(region_sizes[:, None], 1.0)
        region_means = (feature_sums / denom).astype(np.float32)
        ...
        boundary_mask = np.any(neighbor_labels != labels[:, None], axis=1)
        boundary_cells = np.flatnonzero(boundary_mask)
        ...
        accepted = 0
        evaluated = 0
        for cell in boundary_cells.tolist():
            source = int(labels[int(cell)])
            if source < 0 or source >= n_regions or region_sizes[source] <= min_cells:
                continue
            candidate_labels = np.unique(labels[neighbors[int(cell)]])
            candidate_labels = candidate_labels[
                (candidate_labels >= 0) & (candidate_labels != source)
            ]
            if candidate_labels.size == 0:
                continue
            evaluated += 1
            x_feat = feature_view[int(cell)].astype(np.float64)
            x_coord = coords[int(cell)].astype(np.float64)
            source_label = int(region_cluster_labels[source])
            if source_label < 0 or source_label >= prototypes.shape[0]:
                continue
            source_n = float(region_sizes[source])
            source_mean = feature_sums[source] / max(source_n, 1.0)
            source_proto = prototypes[source_label].astype(np.float64)
            source_before = source_n * float(np.mean((source_mean - source_proto) ** 2))
            source_after_mean = (feature_sums[source] - x_feat) / max(
                source_n - 1.0, 1.0
            )
            source_after = (source_n - 1.0) * float(
                np.mean((source_after_mean - source_proto) ** 2)
            )
            source_center = coord_sums[source] / max(source_n, 1.0)
            ...
            for target_raw in candidate_labels.tolist():
                ...
                if target_label < 0 or target_label >= prototypes.shape[0]:
                    continue
                target_n = float(region_sizes[target])
                target_mean = feature_sums[target] / max(target_n, 1.0)
                target_proto = prototypes[target_label].astype(np.float64)
                target_before = target_n * float(
                    np.mean((target_mean - target_proto) ** 2)
                )
                target_after_mean = (feature_sums[target] + x_feat) / max(
                    target_n + 1.0, 1.0
                )
                target_after = (target_n + 1.0) * float(
                    np.mean((target_after_mean - target_proto) ** 2)
                )
                cluster_delta = (source_after + target_after) - (
                    source_before + target_before
                )

                target_center = coord_sums[target] / max(target_n, 1.0)
                spatial_delta = (
                    float(np.sum((x_coord - target_center) ** 2))
                    - float(np.sum((x_coord - source_center) ** 2))
                ) / scale_sq
                new_cut = float(np.mean(own_neighbor_labels != target))
                cut_delta = new_cut - old_cut
                total_delta = (
                    float(cluster_weight) * cluster_delta
                    + float(spatial_weight) * spatial_delta
                    + float(cut_weight) * cut_delta
                )
            ...
            region_sizes[source] -= 1.0
            region_sizes[best_target] += 1.0
            feature_sums[source] -= x_feat
            feature_sums[best_target] += x_feat
            coord_sums[source] -= x_coord
            coord_sums[best_target] += x_coord
            accepted += 1
        ...
        history.append(
            {
                "iteration": float(iteration),
                "boundary_cells": float(boundary_cells.size),
    ...
    refined_members = _members_from_labels(labels)
    connectivity_radius = _estimate_connectivity_radius_um(
        coords, target_scale_um=float(target_scale_um)
    )
    connected_centers, connected_members = _split_members_by_spatial_connectivity(
        coords_um=coords,
        members=refined_members,
        connectivity_radius_um=connectivity_radius,
    )
    connected_ids = [
        np.asarray([idx], dtype=np.int32) for idx in range(len(connected_members))
    ]
    final_centers, final_members, final_ids = _merge_data_driven_regions(
        coords_um=coords,
        centers_um=connected_centers,
        members=connected_members,
        component_ids=connected_ids,
        min_cells=min_cells,
        max_subregions=int(max_subregions),
        partition_features=feature_view,
        feature_weight=max(float(cluster_weight), 0.0),
        target_scale_um=float(target_scale_um),
        max_area_um2=None,
    )
    final_centers, final_members, final_ids = _sort_partition_by_center(
        final_centers, final_members, final_ids
    )
    _validate_mutually_exclusive_memberships(
        n_cells, final_members, require_full_coverage=True
    )
    if history:
        history[-1]["regions_before_connectivity_split"] = float(len(refined_members))
        history[-1]["regions_after_connectivity_split"] = float(len(connected_members))
        history[-1]["regions_after_min_cell_merge"] = float(len(final_members))
    return (
        final_centers.astype(np.float32),
        [np.asarray(member, dtype=np.int32) for member in final_members],
        [np.asarray(ids, dtype=np.int32) for ids in final_ids],
        history,
    )
...
def _merge_data_driven_regions(
    *,
    ...
    total_cells = int(
        sum(np.asarray(member, dtype=np.int32).size for member in members)
    )
    if total_cells < min_cells:
        raise RuntimeError(
            "No valid subregions can be created because the full partition is smaller than min_cells."
        )

    coords = np.asarray(coords_um, dtype=np.float32)
    centers = [
        np.asarray(center, dtype=np.float32)
        for center in np.asarray(centers_um, dtype=np.float32)
    ]
    ...
    while int(np.sum(active)) > 1:
        ...
        sizes = np.asarray(
            [member_list[idx].size for idx in active_idx], dtype=np.int64
        )
        ...
        candidates = [
            idx for idx in adjacency[source] if active[int(idx)] and int(idx) != source
        ]
        if candidates:
        ...
        if max_area_um2 is not None and float(max_area_um2) > 0.0:
            ...
            for candidate in target_pool.tolist():
                ...
                if _member_area_um2(coords, merged_candidate) <= float(max_area_um2) * (
                    1.0 + 1e-6
                ):
            ...
            if feasible_targets:
                ...
                continue
        center_arr = np.vstack([centers[int(idx)] for idx in target_pool]).astype(
            np.float32
        )
        ...
        if feature_centers is not None and float(feature_weight) > 0.0:
            feat_arr = np.vstack(
                [feature_centers[int(idx)] for idx in target_pool]
            ).astype(np.float32)
            feat_delta = feat_arr - feature_centers[source]
            score = score + float(feature_weight) * np.mean(
                feat_delta * feat_delta, axis=1
            )
        target = int(target_pool[int(np.argmin(score))])

        merged_members = np.unique(
            np.concatenate([member_list[target], member_list[source]])
        ).astype(np.int32)
        merged_ids = np.unique(
            np.concatenate([id_list[target], id_list[source]])
        ).astype(np.int32)
    ...
    active_idx = active_indices()
    out_members = [
        np.asarray(member_list[int(idx)], dtype=np.int32) for idx in active_idx.tolist()
    ]
    if (max_area_um2 is None or float(max_area_um2) <= 0.0) and any(
        member.size < min_cells for member in out_members
    ):
        raise RuntimeError("No valid subregions remain after enforcing min_cells.")
    out_ids = [
        np.asarray(id_list[int(idx)], dtype=np.int32) for idx in active_idx.tolist()
    ]
    out_centers = np.vstack([centers[int(idx)] for idx in active_idx.tolist()]).astype(
        np.float32
    )
...
def build_deep_graph_segmentation_subregions(
    ...
    *,
    ...
    np.ndarray, list[np.ndarray], np.ndarray, list[np.ndarray], list[np.ndarray]
]:
    coords = np.asarray(coords_um, dtype=np.float32)
    if coords.shape[0] == 0:
        raise RuntimeError("No cells were provided, so no subregions can be created.")
    if float(target_scale_um) <= 0:
        raise ValueError(
            "target_scale_um must be positive for deep graph segmentation."
        )
    ...
    _geometry_progress(
        f"deep segmentation seed partition produced {len(seed_members)} regions"
    )
    _, refined_members = _refine_partition_by_feature_boundaries(
        ...
        n_neighbors=_deep_segmentation_knn_default()
        if segmentation_knn is None
        else int(segmentation_knn),
    )
    _geometry_progress(
        f"deep segmentation boundary refinement produced {len(refined_members)} regions"
    )
    connectivity_radius = _estimate_connectivity_radius_um(
        coords, target_scale_um=float(area_scale_um)
    )
    _geometry_progress(
        f"splitting {len(refined_members)} regions by spatial connectivity"
    )
    ...
    _geometry_progress(
        f"spatial connectivity split produced {len(basic_members)} basic regions"
    )
    _geometry_progress(
        "max-area is a soft QC target; enforcing min_cells and connected data-driven regions "
        f"({len(basic_members)} basic regions)"
    )
    basic_ids = [np.asarray([idx], dtype=np.int32) for idx in range(len(basic_members))]
    _geometry_progress(
        f"merging {len(basic_members)} basic regions with min_cells={int(min_cells)}"
    )
    subregion_centers, subregion_members, subregion_basic_ids = (
        _merge_data_driven_regions(
            coords_um=coords,
    ...
    _geometry_progress(f"min-cell merge produced {len(subregion_members)} subregions")
    subregion_centers, subregion_members, subregion_basic_ids = (
        _sort_partition_by_center(
            subregion_centers,
            subregion_members,
            subregion_basic_ids,
        )
    ...
    _validate_mutually_exclusive_memberships(
        coords.shape[0], subregion_members, require_full_coverage=True
    )
    return (
...
def build_data_driven_subregions(
    ...
    *,
    ...
    if coords.shape[0] == 0:
        raise RuntimeError("No cells were provided, so no subregions can be created.")
    scale_um = (
        float(target_scale_um) if target_scale_um is not None else float(stride_um)
    )
    ...
    if partition_features is not None and feature_weight > 0.0 and feature_dims > 0:
        ...
        if feature_arr.ndim != 2 or feature_arr.shape[0] != coords.shape[0]:
            raise ValueError(
                "partition_features must have shape (n_cells, n_features)."
            )
        feature_view = _standardize_features(
            feature_arr[:, : min(feature_dims, feature_arr.shape[1])]
        )
    ...
    _geometry_progress(
        f"data-driven seed partition produced {len(seed_members)} regions"
    )
    ...
    _geometry_progress(
        f"data-driven boundary refinement produced {len(seed_members)} regions"
    )
    connectivity_radius = _estimate_connectivity_radius_um(
        coords, target_scale_um=area_scale_um
    )
...
def _validate_mutually_exclusive_memberships(
    ...
    *,
    ...
    for idx, member in enumerate(members):
        member_arr = _coerce_membership_indices(
            member, n_cells=int(n_cells), subregion_index=int(idx)
        )
    ...
    if bool(require_full_coverage) and (counts.size == 0 or int(counts.min()) < 1):
        raise RuntimeError(
            "Constructed subregions do not cover every cell exactly once."
        )
    return counts
...
def _sample_points_in_triangle(
    tri: np.ndarray, n: int, rng: np.random.Generator
) -> np.ndarray:
    u = rng.random((n, 1))
    v = rng.random((n, 1))
    flip = (u + v) > 1.0
...
def _sample_uniform_points_in_convex_hull(
    coords: np.ndarray, n_points: int, seed: int
) -> np.ndarray:
    ...
    if hull_pts.shape[0] < 3:
        take = rng.choice(
            coords.shape[0], size=min(n_points, coords.shape[0]), replace=False
        )
...
def _sample_uniform_points_in_polygon_components(
    ...
    components = [
        ...
        for poly in polygon_components
        if np.asarray(poly).shape[0] >= 3
    ]
    if not components:
        return np.zeros((0, 2), dtype=np.float32)
...
def sample_geometry_points(
    ...
    if region_geometry.mask is not None:
        pts = _sample_uniform_points_in_mask(
            region_geometry.mask,
            n_points=n_points,
            seed=seed,
            affine=region_geometry.affine,
        )
        if pts.shape[0] == 0:
            raise ValueError(
                f"Region '{region_geometry.region_id}' provided an empty mask geometry."
            )
        return pts, "mask", False
    if region_geometry.polygon_components:
        pts = _sample_uniform_points_in_polygon_components(
            region_geometry.polygon_components, n_points=n_points, seed=seed
        )
        if pts.shape[0] > 0:
            return pts, "polygon_components", False
        raise ValueError(
            f"Region '{region_geometry.region_id}' provided polygon components but sampling produced no points."
        )
    if (
        region_geometry.polygon_vertices is not None
        and np.asarray(region_geometry.polygon_vertices).shape[0] >= 3
    ):
        pts = _sample_uniform_points_in_polygon_components(
            [np.asarray(region_geometry.polygon_vertices)], n_points=n_points, seed=seed
        )
        if pts.shape[0] > 0:
            return pts, "polygon", False
        raise ValueError(
            f"Region '{region_geometry.region_id}' provided polygon geometry but sampling produced no points."
        )
    if bool(getattr(region_geometry, "use_observed_geometry", False)):
        pts = _sample_observed_point_cloud_geometry(
            observed_coords, n_points=n_points, seed=seed
        )
        if pts.shape[0] == 0:
            raise ValueError(
                f"Region '{region_geometry.region_id}' has no observed coordinates for data-driven geometry."
            )
    ...
    return (
        _sample_uniform_points_in_convex_hull(
            observed_coords, n_points=n_points, seed=seed
        ),
...
def _subregion_shape_descriptors(coords: np.ndarray) -> dict[str, float]:
    ...
    if hull_pts.shape[0] >= 3:
        ...
        perimeter = float(
            np.sum(
                np.sqrt(np.sum((np.roll(hull_pts, -1, axis=0) - hull_pts) ** 2, axis=1))
            )
        )
    else:
        area = 0.0
        perimeter = float(
            np.sum(
                np.sqrt(
                    np.sum((coords - coords.mean(axis=0, keepdims=True)) ** 2, axis=1)
                )
            )
        )
    ...
    compactness = (
        float(4.0 * np.pi * area / max(perimeter**2, 1e-12)) if perimeter > 0 else 0.0
    )
    return {
...
def _shape_descriptor_frame(
    ...
    if region_geometries is not None and len(region_geometries) != len(
        subregion_members
    ):
        raise ValueError(
            "region_geometries must have the same length as subregion_members for shape descriptors."
        )
    ...
    for rid, members in enumerate(subregion_members):
        ...
        if large_observed_only and member_arr.size <= 3:
            if member_arr.size == 0:
                ...
                perimeter = float(
                    np.sum(
                        np.sqrt(np.sum((np.roll(pts, -1, axis=0) - pts) ** 2, axis=1))
                    )
                )
                ...
                if np.allclose(centered, 0.0):
                    ...
                    eigvals = np.sort(np.maximum(np.linalg.eigvalsh(cov), 1e-12))[::-1]
                    aspect_ratio = float(
                        np.sqrt(eigvals[0]) / max(float(np.sqrt(eigvals[1])), 1e-8)
                    )
                    eccentricity = float(
                        np.sqrt(max(0.0, 1.0 - eigvals[1] / max(eigvals[0], 1e-12)))
                    )
                desc = {
                    "shape_area": float(area),
                    "shape_perimeter": float(perimeter),
                    "shape_compactness": float(
                        4.0 * np.pi * area / max(perimeter**2, 1e-12)
                    )
                    if perimeter > 0
                    else 0.0,
            ...
            if region.mask is not None:
                geom = _sample_uniform_points_in_mask(
                    region.mask, n_points=512, seed=rid, affine=region.affine
                )
                if geom.shape[0] > 0:
                ...
                pts = np.vstack(
                    [
                        np.asarray(poly, dtype=np.float64)
                        for poly in region.polygon_components
                        if np.asarray(poly).shape[0] >= 3
                    ]
                )
                if pts.shape[0] > 0:
                ...
                region.polygon_vertices is not None
                and np.asarray(region.polygon_vertices).shape[0] >= 3
            ):
                desc = _subregion_shape_descriptors(
                    np.asarray(region.polygon_vertices, dtype=np.float64)
                )
                source = "explicit_polygon"
...
def _shape_leakage_balanced_accuracy(
    shape_df: pd.DataFrame, labels: np.ndarray, seed: int
) -> float | None:
    if shape_df.empty:
        return None
    y = np.asarray(labels, dtype=np.int32)
    ...
    numeric_cols = [
        ...
        if c != "subregion_id" and np.issubdtype(shape_df[c].dtype, np.number)
    ]
    x = shape_df[numeric_cols].to_numpy(dtype=np.float32)
    cv = StratifiedKFold(n_splits=n_splits, shuffle=True, random_state=seed)
    clf = RandomForestClassifier(
        n_estimators=_leakage_rf_estimators(), random_state=seed
    )
...
def _shape_leakage_permutation_baseline(
    ...
    for _ in range(int(n_perm)):
        score = _shape_leakage_balanced_accuracy(
            shape_df, rng.permutation(labels), seed=int(rng.integers(1_000_000))
        )
...
def _validate_fit_inputs(
    ...
    if min_scale <= 0 or max_scale <= 0 or min_scale > max_scale:
        raise ValueError(
            "min_scale and max_scale must be positive and min_scale <= max_scale."
        )
...
def _gpu_balanced_sinkhorn_transport(
    ...
    if not bool(torch.isfinite(t).all().item()):
        ...
        for fallback_eps in (
            max(float(eps_geom), 1e-5) * 4.0,
            max(float(eps_geom), 1e-5) * 16.0,
        ):
            t, transport_cost, converged, _ = sinkhorn_balanced_log_torch(
                a_t,
                b_t,
            ...
            raise RuntimeError(
                "GPU balanced Sinkhorn produced non-finite plan even after regulariser fallback."
            )
    return (
...
def fit_ot_shape_normalizer(
    ...
    if g.shape[0] < 1:
        raise ValueError(
            "At least 1 geometry point is required for shape normalization."
        )
    ...
    try:
        use_gpu = (
            compute_device is not None and torch.device(compute_device).type == "cuda"
        )
        if use_gpu:
            ...
            sinkhorn_converged = bool(
                err_hist.size == 0 or err_hist[-1] < _cpu_sinkhorn_tol()
            )
        ...
        interpolation_residual = float(
            np.sqrt(np.mean(np.sum((pred - g_mapped) ** 2, axis=1)))
        )
